...
const DashboardPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await getDashboardData(topicId);
        if (res.data?.data) {
  ...
  console.log("Chart Data:", curveData);

  // Zone colors
  const isRed = currentRetention < 50;
  const isYellow = currentRetention >= 50 && currentRetention < 70;
  const zoneColor = isRed ? "#ef4444" : isYellow ? "#eab308" : "#22c55e";
  const zoneName = isRed ? "Red Zone" : isYellow ? "Yellow Zone" : "Green Zone";

  // Review countdown
  const safeHValue = Math.max(0.1, Number(h || 24));
  const hoursElapsed = lastCalculated ? (Date.now() - new Date(lastCalculated).getTime()) / (1000 * 60 * 60) : 0;
  const rawReviewIn = safeHValue - hoursElapsed;
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-slate-900/40 border border-white/5 rounded-xl p-5">
            ...
            <p className="text-3xl font-bold" style={{ color: zoneColor }}>{currentRetention.toFixed(1)}%</p>
          ...
          <div className="bg-slate-900/40 border border-white/5 rounded-xl p-5">
            ...
            <p className="text-3xl font-bold text-blue-400">{M.toFixed(0)}%</p>
          </div>
          <div className="bg-slate-900/40 border border-white/5 rounded-xl p-5">
            <p className="text-xs text-slate-500 mb-1">Half-Life (h)</p>
            <p className="text-3xl font-bold text-purple-400">{h.toFixed(0)}<span className="text-base font-normal text-slate-500 ml-1">hours</span></p>
          </div>
          <div className="bg-slate-900/40 border border-white/5 rounded-xl p-5">
